...
#if defined(NDEBUG)
# error "Chauffeurcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd87eab461a1e0fbcb068ee9f653cdf39eacbeb491ceea759e26e971c7a3f5504");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Chauffeurcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Chauffeurcoin Signed Message:\n";
...
{
    // Chauffeurcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 50 * COIN; //50 -> 50

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 1314000); // Chauffeurcoin: 5 years, BlockHalving
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Chauffeurcoin: 1 day, BlockDifficulty
static const int64 nTargetSpacing = 2 * 60; // Chauffeurcoin: 2 minutes, BlockTime
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Chauffeurcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Chauffeurcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[1] = 0xcc; //c1
        pchMessageStart[2] = 0xb2; //b7
        pchMessageStart[3] = 0xdd; //dc
        hashGenesisBlock = uint256("0x6c0c47ee50ff16b183beb3e4391c5603b276282f2ebe5ebf20b0aa975e8d96d0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CHAUFFEURCOIN IS DESIGNATED DRIVER SERVICE";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04aed7309b39522f599c0d195b31c9a89e5c2e95f3302b7e040808841d7040eae5e0f9e50759568f2b3ff45f8038e44e0e8b37d2f46838d06a179161af0a78d6d2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1525675132;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085435225;

        if (fTestNet)
        {
            block.nTime    = 1525675095;
            block.nNonce   = 394893757;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x26c479510423c034ac90fbc88ae31e4b344ff4dc0c8466e179f8b84d43ea27bf"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf5, 0xcf, 0xbb, 0xd4 }; // Chauffeurcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ChauffeurcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ChauffeurcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ChauffeurcoinMiner(CWallet *pwallet)
{
    printf("ChauffeurcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("chauffeurcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ChauffeurcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ChauffeurcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ChauffeurcoinMiner, pwallet));
}
